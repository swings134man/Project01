...
//		int memberBusiness = 1;					// 세션 비지니스 인트값 가져올것.
		...
		String tId = (String)session.getAttribute("memberId"); //세션 테스트용 아이디
		...
		$('#bt1').click(function() {
			...
			$.ajax({
				...
				data: {
					...
					ad_Content : commentM , 
					ad_Writer : '<%= session.getAttribute("memberId") %>' ,
					ad_Time : ad_Time1 
				},
				success: function(result) {
					if (result == 1) {
							location.href = "http://localhost:8541/project1plus/post.jsp?ad_Writer=<%=dto2.getAd_Writer()%>"
...
<div id="total">
	...
	<script type="text/javascript">
		function like1() {
			var con = confirm('해당 가게를 추천 하시겠습니까?');
			  if (con) {
				location.href="http://localhost:8541/project1plus/recommend.jsp?ad_Name=" + '<%= dto2.getAd_Name() %>';
				alert('추천이 완료되었습니다!');
			} 
		} //추천 func end
	</script>
	...
	<table border="1" style="font-size: 15px; width: 900px;">
	 ...
	 <!-- 지도 ,,, -->
		...
		<script type="text/javascript" src="//dapi.kakao.com/v2/maps/sdk.js?appkey=	58be3508598752845b1ecb8661a8210a"></script>
		<script>
	...
	%> 
			<a href="http://localhost:8541/project1plus/insertU.jsp?ad_Writer=<%= dto2.getAd_Writer()%>">
			<button>수정</button></a>
